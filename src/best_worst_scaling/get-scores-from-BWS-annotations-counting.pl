...
while (my $data = $csv->getline_hr($fh)) {
	...
	if(!defined $items_in_tuple{$best_item}) {
		...
		print STDERR "Best item: $best_item\n";
		print STDERR "Tuple items: ".join(", ", keys %items_in_tuple)."\n";
		exit();
	}
...
foreach my $item (keys %count_item) {
    ...
    my $raw_score = ($count_best{$item} - $count_worst{$item}) / $count_item{$item};

    # Transform score from -1..1 to 1..5
    $scores{$item} = 2 * ($raw_score) + 3;
